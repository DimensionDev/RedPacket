...
describe("Test Create RedPacket function for FT Tokens", () => {
  ...
  it("Should emit CreationSuccess when everything is ok", async () => {
    ...
    const logs = await ethers.provider.getLogs(redpacket.filters.CreationSuccess());
    const parsedLogs = rpInterface.parseLog(logs[0]);
    const results = parsedLogs.args;
    expect(results).to.have.property("total")
      .that.to.be.eq(creationParams.totalTokens.toString());
    expect(results).to.have.property("name")
      .that.to.be.eq(creationParams.name);
    expect(results).to.have.property("message")
