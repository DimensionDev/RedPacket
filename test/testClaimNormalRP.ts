...
describe("Test claim redpacket function for FT tokens", () => {
  ...
  before(async () => {
    ...
    signerAddresses = await Promise.all(signers.map(async (signer): Promise<string> => {
  ...
  it("Should throw error when redpacket id does not exist", async () => {
    const claimParams = await createClaimParam(utils.hexlify(utils.toUtf8Bytes("not exist")), signerAddresses[2], signerAddresses[2]);
    await expect(
      redpacket.connect(signers[2]).claim.apply(null, Object.values(claimParams))
  ...
  it("Should throw error when expired", async () => {
    ...
    const pkt_id = createSuccess.args.id;
    const claimParams = await createClaimParam(pkt_id, signerAddresses[2], signerAddresses[2]);
    await expect(
      redpacket.connect(signers[2]).claim.apply(null, Object.values(claimParams))
  ...
  it("Should throw error when out of stock", async () => {
    ...
    const pkt_id = createSuccess.args.id;
    const claimParams = await createClaimParam(pkt_id, signerAddresses[2], signerAddresses[2]);
    const anotherParams = await createClaimParam(pkt_id, signerAddresses[3], signerAddresses[3]);
    await redpacket.connect(signers[2]).claim.apply(null, Object.values(claimParams));
    await expect(
      redpacket.connect(signers[3]).claim.apply(null, Object.values(anotherParams))
    ).to.be.revertedWith(getRevertMsg("Out of stock"));
  });

  it("Should throw error when password is wrong", async () => {
    const createSuccess = await createRedpacket(0, creationParams);
    const pkt_id = createSuccess.args.id;
    const wrongClaimParams = await createClaimParam(pkt_id, signerAddresses[3], signerAddresses[3]);
    await expect(
      redpacket.connect(signers[2]).claim.apply(null, Object.values(wrongClaimParams))
    ).to.be.revertedWith(getRevertMsg("Verification failed"));
  })
  ...
  it("Should throw error when already claimed", async () => {
    ...
    await expect(
      redpacket.connect(signers[2]).claim.apply(null, Object.values(claimParams))
    ).to.be.revertedWith(getRevertMsg("Already claimed"));
  });
  ...
  it("Should BurnToken single-token redpacket work", async () => {
    ...
    for (const i of [2, 3, 4, 5]) {
      const claimParam = await createClaimParam(pkt_id, signerAddresses[i], signerAddresses[i]);
  ...
  it("Should claim BurnToken work", async () => {
    ...
    expect(BigNumber.from(burnTokenCreationParams.totalTokens).gt(
      claimedValues[0]
        .add(claimedValues[1])
        .add(claimedValues[2])
        .add(claimedValues[3])
  ...
  it("Should claim average amount if not set random", async () => {
    ...
    const pkt_id = createSuccess.args.id;
    const claimParams1 = await createClaimParam(pkt_id, signerAddresses[2], signerAddresses[2]);
    await redpacket.connect(signers[2]).claim.apply(null, Object.values(claimParams1));
    const claimParams2 = await createClaimParam(pkt_id, signerAddresses[3], signerAddresses[3]);
  ...
  it("Should claim random amount if set random", async () => {
    ...
    const claimedValues = claimEvents.map((event) => event.args.claimed_value);
    expect(lodash.take(claimedValues, 3).every((v) => v.toString() === claimedValues[3].toString())).to.be.false;
    expect(BigNumber.from(randomCreationParams.totalTokens).eq(
      claimedValues[0]
        .add(claimedValues[1])
        .add(claimedValues[2])
        .add(claimedValues[3])
  ...
  async function testSuitCreateAndClaimManyRedPackets(claimers: number, largeScaleCreationParams: any) {
    ...
    const pkt_id = createSuccess.args.id
    await Promise.all(
      lodash.range(100).map(async (i) => {
        const claimParams = await createClaimParam(pkt_id, signerAddresses[i], signerAddresses[i])
